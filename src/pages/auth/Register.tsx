...
import { Mail, Lock, Phone, User, ArrowRight, MapPin } from 'lucide-react';
...
export const Register = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const success = await login('sarah@example.com', 'customer', 'password');
      if (!success) { setIsLoading(false); return; }
      toast('Registration successful', 'success');
      setTimeout(() => navigate('/customer/dashboard', { replace: true }), 10);
    } catch (e) {
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 transition-colors duration-200 bg-bg-secondary dark:bg-bg-card">
      <div className="max-w-md w-full space-y-8 bg-white dark:bg-charcoal p-8 rounded-3xl border border-border-primary shadow-xl">
        <div className="text-center">
          ...
          <h2 className="text-3xl font-black text-primary-text tracking-tight">Create Account</h2>
          <p className="mt-2 text-sm text-secondary-text">Join ServiceHub to book services</p>
        </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="relative mt-1">
                ...
                <input type="text" required className="w-full pl-10 pr-4 py-3 bg-bg-secondary dark:bg-charcoal border border-border-primary rounded-xl text-primary-text focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all" placeholder="Sarah Jenkins" />
              </div>
            </div>
            <div>
              ...
              <div className="relative mt-1">
                ...
                <input type="email" required className="w-full pl-10 pr-4 py-3 bg-bg-secondary dark:bg-charcoal border border-border-primary rounded-xl text-primary-text focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all" placeholder="sarah@example.com" />
              </div>
            </div>
            <div>
